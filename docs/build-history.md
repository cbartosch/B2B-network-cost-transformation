# Build history and audit log

Every remediation pass, in order, with the reasoning behind each decision. Moved out of
`README.md` at build 4.31.0: 56 build entries across 2,550 lines is useful history and a
poor introduction, and it was crowding out the things a newcomer actually needs.

Read this when you want to know **why** something is the way it is. The short answer to
"what is this and how do I run it" is in `README.md`.

A note on what these entries are. Most were written after finding a defect by executing
something that had never run. They name what broke, what it would have cost, and — where
it applies — which earlier claim of mine turned out to be wrong. That last part is
deliberate: a log that only records successes teaches nothing about where to look next.
