...
def _migrate_v63(conn) -> None:
    """reference.archetype_resilience - criticality that reaches the estate.

    The industry benchmark's criticality tier, committed share and dual-access
    probability applied only where its representative archetype appeared in
    the industry's estate mix, and it does not for 43 of 47 industries. Three
    industries returned byte-identical baselines.

    Created by create_all; the rows come from the seed.
    """
    log.info("v63: archetype_resilience created by create_all")
