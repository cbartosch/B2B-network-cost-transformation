...
def _migrate_v48(conn) -> None:
    """The observation carries what the rate card is keyed on.

    A cleared benchmark derives a prior. Since 4.168 the rate card is keyed on
    service class and access technology, and the observation carried neither -
    so every derived prior was reachable only through the legacy fallback, and
    the route from evidence to a graded rate was broken by the re-key that
    ...
    for product, (service_class, technology) in LEGACY.items():
        moved += conn.execute(text(
            "UPDATE benchmark.benchmark_observation "
            "SET service_class = :sc, access_technology = :at "
            "WHERE product = :p AND service_class IS NULL"),
            {"sc": service_class, "at": technology, "p": product}).rowcount or 0
    log.info("v48: %d column(s) added, %d observation(s) given a service "
             "class", added, moved)


def _migrate_v49(conn) -> None:
    """archetype_prior.committed_fraction.

    The model knew one bandwidth per site type and could not say whether it was
    the pipe or the guarantee. On a GB rate card that is 980 against 420 a
    month - the baseline 2.3x too high on every committed circuit.
    """
    added = _add_column(conn, db.archetype_prior, "committed_fraction")
    log.info("v49: committed_fraction added=%s", bool(added))


def _migrate_v50(conn) -> None:
    """case.committed_fraction_by_archetype.

    The seeded fraction is a default, not a decision. An engagement that knows
    what its sites actually commit should be able to say so without a rebuild -
    the same treatment the service class already gets.
    """
    added = _add_column(conn, db.case, "committed_fraction_by_archetype")
    log.info("v50: committed_fraction_by_archetype added=%s", bool(added))


def _migrate_v51(conn) -> None:
    """outside_in.validation_case.

    The harness could compare an estimate against an outturn since 4.160 and
    the corpus stayed empty, because nothing produced a case. A folder somebody
    has to remember to fill is not a workflow.

    create_all builds the table; this step exists so the version advances and
    the reconciler verifies it.
    """
    log.info("v51: validation_case created by create_all and reconciled")


def _migrate_v52(conn) -> None:
    """outside_in.assumption_register and outside_in.data_request.

    Specification 0.4, the two Stage 0 prepopulation targets that were missing.
    A gap that is not written down is a gap somebody has to notice again next
    week.
    """
    log.info("v52: assumption_register and data_request created by "
             "create_all and reconciled")


def _migrate_v53(conn) -> None:
    """outside_in.provider and outside_in.product.

    Specification 0.4. Research domain 8 gathers exactly this - which carriers
    and partners the entity uses - and promotion had nowhere to put the answer,
    so every finding about a supplier was displayed once and lost.
    """
    log.info("v53: provider and product created by create_all")


def _migrate_v54(conn) -> None:
    """analysis.estimate_delta - specification 0.5D.

    Additive. Nothing existing writes or reads it, and a snapshot without a
...
def _migrate_v56(conn) -> None:
    """reference.industry_benchmark - the supplied BICS L3 WAN benchmark.

    The first reference data here that was not invented in this repository.
    Its own table rather than merged into the seeded archetype rows, because
    folding an external benchmark into our own judgement would make the two
    indistinguishable.
    """
    log.info("v56: industry_benchmark created by create_all")


def _migrate_v57(conn) -> None:
    """outside_in.case_rate - a client's own rates.

    All eleven reference tables the estimate reads were global, so two clients
    in the same industry and country got the same rate card and only their
    footprints differed. A client's invoices are the best evidence this model
    can have and there was nowhere to put them.
    """
...
def _migrate_v59(conn) -> None:
    """case.site_rule - what counts as a site.

    A site count is the largest single driver of the baseline and was the one
    number with no stated basis. A footprint without its rule is an assertion,
    and the model refuses assertions everywhere else.

    Null on existing cases, which reads as "not declared" and is reported
    rather than assumed: a rule invented for a case that never had one would
    put a basis on a count nobody chose.
    """
    added = _add_column(conn, db.case, "site_rule")
    log.info("v59: site_rule added=%s", bool(added))


def _migrate_v60(conn) -> None:
    """benchmark_observation.value_qualifier and term_months_basis.

    A run failed closed three times on "over 100" in known_fact.corroborate.
    The same gap existed in the benchmark extractor, which feeds the rate card:
    a tariff saying "from GBP 250 a month" had a Decimal field and nowhere to
    put the "from".

    Existing rows default to EXACTLY, which is what every observation before
    this meant.
    """
    added = _add_column(conn, db.benchmark_observation, "value_qualifier")
    added_basis = _add_column(conn, db.benchmark_observation,
                              "term_months_basis")
    log.info("v60: value_qualifier=%s term_months_basis=%s",
...
def _migrate_v62(conn) -> None:
    """reference.fx_rate - exchange rates as governed reference data.

    A GBP case against a USD rate card refused to price, because nothing
    converted between them. `currency.convert()` had been written and called
    by nothing, and `fx_convention` had been collected by pre-flight and read
    by no calculation.
    """
    log.info("v62: fx_rate created by create_all")
...
MIGRATIONS = {2: _migrate_v2, 3: _migrate_v3, 4: _migrate_v4, 5: _migrate_v5,
              ...
              21: _migrate_v21, 22: _migrate_v22, 23: _migrate_v23, 24: _migrate_v24, 25: _migrate_v25, 26: _migrate_v26, 27: _migrate_v27, 28: _migrate_v28, 29: _migrate_v29, 30: _migrate_v30, 31: _migrate_v31, 32: _migrate_v32, 33: _migrate_v33, 34: _migrate_v34, 35: _migrate_v35, 36: _migrate_v36, 37: _migrate_v37, 38: _migrate_v38, 39: _migrate_v39, 40: _migrate_v40, 41: _migrate_v41, 42: _migrate_v42, 43: _migrate_v43, 44: _migrate_v44, 45: _migrate_v45, 46: _migrate_v46, 47: _migrate_v47, 48: _migrate_v48, 49: _migrate_v49, 50: _migrate_v50, 51: _migrate_v51, 52: _migrate_v52, 53: _migrate_v53, 54: _migrate_v54, 55: _migrate_v55, 56: _migrate_v56, 57: _migrate_v57, 58: _migrate_v58, 59: _migrate_v59, 60: _migrate_v60, 61: _migrate_v61, 62: _migrate_v62}
